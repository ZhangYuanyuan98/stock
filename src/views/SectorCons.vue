<template>
    <div id="app">
        ...
        <el-container style="width: 100%;border: 1px solid #eee ;display: flex; justify-content: center;">
            ...
            <SideBar />
            <el-main >
              <h1 class="custom-text-center">{{ boardName }}</h1>
                <!-- 表格 -->
...
export default{
  ...
  methods:{
    ...
    async fetchData() {
      ...
      if (this.sectorType === 'concept') {
        endpoint = '/api/board_concept_cons';
        } else if (this.sectorType === 'industry') {
        endpoint = '/api/board_industry_cons';
        } 
      const result = await fetchList(params, endpoint);
      const boardName = result.data.sector_name;
  ...
  initCandlestickCharts() {
    ...
    const maxDataValue = Math.max(...item.flow_data.map(data => data['流入资金净额']));
